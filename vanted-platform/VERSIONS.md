...
| TypeScript | 6.0.3 | Required by Angular 22.1; stay within the >=6.0 <6.1 range |
| Node.js | 24.19.0 LTS | Pin/update within the supported Angular 22 Node line |
| Java | 25.0.4.1 LTS | Apply current security updates |
| Spring Boot | 4.0.8 GA | Use GA releases only |
| MySQL | 8.4.11 LTS image | Pin an existing official Docker tag; update after registry verification |
...
4. Docker images are explicitly versioned and must refer to tags that actually exist in the image registry; never use `latest`.
...
The goal is stability first: newest verified supported GA release, not a guessed or unreleased version.
